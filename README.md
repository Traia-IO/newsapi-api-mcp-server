# Newsapi API MCP Server

This is an MCP (Model Context Protocol) server that provides with authentication via Bearer tokens access to the Newsapi API API. It enables AI agents and LLMs to interact with Newsapi API through standardized tools.

## Features

- 🔧 **MCP Protocol**: Built on the Model Context Protocol for seamless AI integration
- 🌐 **Full API Access**: Provides tools for interacting with Newsapi API endpoints
...
- **Newsapi API Website**: [https://newsapi.org/v2](https://newsapi.org/v2)
...
*Note: Replace `example_tool` with actual Newsapi API API tools based on the documentation.*
...
1. Clone this repository:
   ```bash
   git clone https://github.com/Traia-IO/newsapi-api-mcp-server.git
   cd newsapi-api-mcp-server
   ```

2. Set your API key:
   ```bash
   export NEWSAPI_API_KEY="your-api-key-here"
   ```
...
2. Run the server:
   ```bash
NEWSAPI_API_KEY="your-api-key-here" uv run python -m server
   ```
...
Clients with their own Newsapi API API key can use the server for free:

```bash
# Request with client's API key
curl -X POST http://localhost:8000/mcp \
  -H "Authorization: Bearer CLIENT_NEWSAPI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"method":"tools/call","params":{"name":"example_tool","arguments":{"query":"test"}}}'
```

**Flow**:
1. Client provides their Newsapi API API key
2. Server uses client's API key to call Newsapi API API
3. No payment required
4. Client pays Newsapi API directly
...
6. Server uses its INTERNAL Newsapi API API key to call the API
...
```bash
# Required
NEWSAPI_API_KEY=your_internal_newsapi-api_api_key  # Server's API key (for payment mode)
...
1. **Client Decision**:
   - Has Newsapi API API key? → Mode 1 (Authenticated)
...
1. Create API client functions for Newsapi API endpoints
...
```json
{
  "github_url": "https://github.com/Traia-IO/newsapi-api-mcp-server",
  "mcp_server": {
    "name": "newsapi-api-mcp",
    "description": "Deployment of newsapi - news aggregation from 80,000+ sources",
...
- `NEWSAPI_API_KEY`: Your Newsapi API API key (required)
## Troubleshooting
