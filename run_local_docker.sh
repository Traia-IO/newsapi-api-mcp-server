...
# Script to build and run the Newsapi API MCP Server locally in Docker
...
IMAGE_NAME="newsapi-api-mcp-server"
CONTAINER_NAME="newsapi-api-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running Newsapi API MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# Newsapi API MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
if ! grep -q "^NEWSAPI_API_KEY=[^[:space:]]" .env; then
    if [ -t 0 ]; then
        echo ""
        echo -e "${YELLOW}📝 Please enter your Newsapi API API key:${NC}"
        echo -e "${YELLOW}   (This is the server's internal API key used when clients pay via 402)${NC}"
        read -p "NEWSAPI_API_KEY: " API_KEY_VALUE
        if [ -n "$API_KEY_VALUE" ]; then
            if grep -q "^NEWSAPI_API_KEY=" .env; then
                if [[ "$OSTYPE" == "darwin"* ]]; then
                    sed -i '' "s|^NEWSAPI_API_KEY=.*|NEWSAPI_API_KEY=$API_KEY_VALUE|" .env
                else
                    sed -i "s|^NEWSAPI_API_KEY=.*|NEWSAPI_API_KEY=$API_KEY_VALUE|" .env
                fi
            else
                echo "NEWSAPI_API_KEY=$API_KEY_VALUE" >> .env
            fi
        ...
        echo -e "${YELLOW}⚠️  Non-interactive mode: Please set NEWSAPI_API_KEY in .env manually${NC}"
    fi
else
    echo -e "${GREEN}✅ NEWSAPI_API_KEY already set in .env${NC}"
fi
...
echo
echo -e "${GREEN}✅ Newsapi API MCP Server is running!${NC}"
echo
