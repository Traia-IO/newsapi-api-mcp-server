# Newsapi API MCP Server Implementation Guide

You are working on implementing the Newsapi API MCP (Model Context Protocol) server. The basic structure has been set up, and your task is to implement the actual API integration.

## API Information

- **API Name**: Newsapi API
- **Documentation**: [https://newsapi.org/docs](https://newsapi.org/docs)
- **Website**: [https://newsapi.org/v2](https://newsapi.org/v2)
- **SDK**: `newsapi-python` (already included in pyproject.toml)
- **Authentication**: Required - API key via `NEWSAPI_API_KEY` environment variable
...
When a client provides their own Newsapi API API key:
...
Flow:
  1. Client connects with their Newsapi API API key
  2. MCP server uses client's API key to call Newsapi API API
  3. No payment required
  4. Client pays Newsapi API directly (not the MCP server)
```

**Use Case**: Clients who already have a Newsapi API subscription/API key
...
Flow:
  ...
  4. MCP server uses its INTERNAL Newsapi API API key to call the API
  5. Client pays the MCP server (not Newsapi API)
```

**Use Case**: Pay-per-use clients without their own Newsapi API subscription
...
async def your_tool_name(context: Context, param1: str) -> Dict[str, Any]:
    ...
    if api_key:
        ...
        api_key_to_use = os.getenv("NEWSAPI_API_KEY")
    
    # Call API
    headers = {"Authorization": f"Bearer {api_key_to_use}"}
    response = requests.get("https://newsapi.org/v2/endpoint", headers=headers)
...
**Required**:
- `NEWSAPI_API_KEY`: Server's internal Newsapi API API key (used when clients pay via 402)
...
NEWSAPI_API_KEY=your_newsapi-api_api_key_here
...
```json
{
  "mcp_server": {
    "capabilities": [
      ...
      "search_newsapi_api",
      "get_newsapi_api_info",
      // Add all other tools you implement
    ]
  },
  "tags": ["newsapi api", "api", /* add relevant tags like "search", "data", etc. */]
}
```
...
Add functions to call the Newsapi API API with retry support. Example pattern:
...
def call_newsapi_api_api(endpoint: str, params: Dict[str, Any], api_key: str) -> Dict[str, Any]:
    """Call Newsapi API API using the SDK with automatic retry."""
    # Initialize the SDK client
    client = newsapi_python.Client(api_key=api_key)
    ...
    try:
        ...
        raise Exception(f"Newsapi API API error: {str(e)}")
```
...
```python
@mcp.tool()
async def search_newsapi_api(
    context: Context,
    query: str,
    limit: int = 10
) -> Dict[str, Any]:
    """
    Search Newsapi API for [specific data type].
    ...
    try:
        # The call_newsapi_api_api function already has retry logic
        results = call_newsapi_api_api(
...
```python
@mcp.tool()
async def get_newsapi_api_info(
    context: Context,
    item_id: str
) -> Dict[str, Any]:
    """
...
```python
@mcp.tool()
async def create_newsapi_api_item(
    context: Context,
    name: str,
    data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    Create a new item in Newsapi API.
...
3. **Add Tool Examples** in README.md:
   ...
   result = await tool.search_newsapi_api(query="example", limit=5)
   ```
...
### 9. Common Newsapi API Use Cases

Based on the API documentation, consider implementing tools for these common use cases:

1. **TODO**: List specific use cases based on Newsapi API capabilities
2. **TODO**: Add more relevant use cases
3. **TODO**: Include any special features of this API

### 10. Example API Calls

Here are some example API calls from the Newsapi API documentation that you should implement:
...
- Check the Newsapi API API documentation: https://newsapi.org/docs
- Review the MCP specification: https://modelcontextprotocol.io
- Look at other MCP server examples in the Traia-IO organization

Remember: The goal is to make Newsapi API's capabilities easily accessible to AI agents through standardized MCP tools. 