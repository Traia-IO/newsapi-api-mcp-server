#!/usr/bin/env python3
"""
Newsapi API MCP Server - FastMCP with D402 Transport Wrapper
...
- NEWSAPI_API_KEY: Server's internal API key (for paid requests)
...
logger = logging.getLogger('newsapi-api_mcp')
...
if not API_KEY:
    logger.warning(f"⚠️  NEWSAPI_API_KEY not set - payment required for all requests")

logger.info("="*80)
logger.info(f"Newsapi API MCP Server (FastMCP + D402 Wrapper)")
...
mcp = FastMCP("Newsapi API MCP Server", host="0.0.0.0")
...
# - NEWSAPI_API_API_KEY: Server's internal API key for payment mode
#
...
@require_payment_for_tool(
    ...
    description="Get breaking news headlines. You must provide at l"
...
async def top_headlines(
    ...
    """
    Get breaking news headlines. You must provide at least one of: country, category, or sources.

    Generated from OpenAPI endpoint: GET /top-headlines

    Args:
        context: MCP context (auto-injected by framework, not user-provided)
        q: Keywords or phrases to search for in the headline and body. (optional) Examples: "elections", "AI regulation"
        country: 2-letter ISO 3166-1 country code. (optional) Examples: "us", "ng"
        category: News category. (optional) Examples: "technology", "business"
        sources: Comma-separated source IDs (cannot be mixed with country or category). (optional) Examples: "bbc-news,cnn"
        pageSize: Number of results per page (1–100). (optional, default: 20)
        page: Page number (>= 1). (optional, default: 1)
    ...
    Example Usage:
        # Minimal (required params only):
        await top_headlines(country="us")
    ...
    try:
        ...
        logger.error(f"Error in top_headlines: {e}")
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="newsapi-api-mcp-server"  # MCP server ID for tracking
    )
    ...
    async def health_check(request: Request) -> JSONResponse:
        ...
        return JSONResponse(
            content={
                "status": "healthy",
                "service": "newsapi-api-mcp-server",
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting Newsapi API MCP Server")
    logger.info("="*80)
